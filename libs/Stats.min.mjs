...
Stats.customInit = function ({ max_fps, elem, elem_lt }) {
    const a2e = a => { a.slice(1).forEach(e => e(a[0])); return a[0]; };
    max_fps ??= 60;
    elem_lt ??= [0, 0];
    elem ??= a2e([
        document.createElement("div"),
        e => e.style.position = "absolute",
        e => e.style.left = elem_lt[0] + "px",
        e => e.style.top = elem_lt[1] + "px",
        e => document.body.appendChild(e),
    ]);
    const setupStats = () => {
        const stats = new Stats();
        // 更新頻度の高い独自のFPSを表示するパネルを追加する。
        // 描画は、stats.customFpsPanel.updateによりfps値は独自に計算したものを与える。
        ...
        elem.appendChild([
            document.createElement("div"),
            e => e.appendChild(document.createTextNode(` y軸の最大値 ${max_fps}fps`)),
            e => e.style.cssText = "display:inline-block",
        ].a2e());
        return stats;
    };
    const stats = setupStats();
    stats.customUpdate = (fps) => {
        stats.update();
        stats.customFpsPanel.update(fps, max_fps);
    }
    return stats;
};
